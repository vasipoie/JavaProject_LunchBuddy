...
public class MemberService {
	...
	public boolean idcheck(String id) {
		...
		if(!(id.matches("^[a-zA-Z0-9]*$"))) { //영어, 숫자 아닌 문자가 입력
			Controller.sessionStorage.put("error message", "id는 영어 혹은 숫자만 입력 가능합니다.");
		}else if(id.length()<5||15<id.length()) { //입력 된 id의 길이가 5~15를 벗어남
			Controller.sessionStorage.put("error message", "id는 5~15자로 입력해주세요.");
		}else if(dao.idcheck(id)!=null) { // 기존에 있는 id인지 확인
			Controller.sessionStorage.put("error message","사용 중인 id입니다." );
		} else return true;
		return false;
	}

	//Controller.sessionStorage.put("error message", );
	public boolean pwcheck(String pw) {
		// 8자 이상 15자 이하의 영어, 숫자
		if(!(pw.matches("^[a-zA-Z0-9]*$"))) { //영어, 숫자 아닌 문자가 입력
			Controller.sessionStorage.put("error message","pw는 영어 혹은 숫자만 입력 가능합니다." );
		}else if(pw.length()<8||15<pw.length()) { //입력 된 pw의 길이가 8~15를 벗어남
			Controller.sessionStorage.put("error message","pw는 8~15자로 입력해주세요." );
		}else return true;
		return false;
	}
	...
	public boolean namecheck(String name) {
		// 2~10자의 한글
		if(!(name.matches("^[가-힣]*$"))) {
			Controller.sessionStorage.put("error message","이름은 한글만 입력 가능합니다." );
		}else if(name.length()<2||name.length()>10) {
			Controller.sessionStorage.put("error message","이름은 2~10자만 입력 가능합니다." );
		}else return true;
		return false;
	}
	...
	public boolean phonecheck(String phone) {
		...
		if(!(phone.matches("^[0-9]*$"))) {
			Controller.sessionStorage.put("error message","숫자만 입력하세요." );
		}else if(phone.length()<10||phone.length()>15) {
			Controller.sessionStorage.put("error message", "10~15자의 숫자를 입력하세요.");
		}else return true;
		return false;
	}
	...
	public boolean nickcheck(String nick) {
		...
		if(!nick.matches("^[a-zA-Z0-9가-힣]*$")) {
			Controller.sessionStorage.put("error message", "한글, 영어, 숫자만 입력해주세요.");
		}else if(nick.length()<2||nick.length()>5) {
			Controller.sessionStorage.put("error message", "닉네임은 2~5자입니다.");
		}else return true;
		return false;
	}
